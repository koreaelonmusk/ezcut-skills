---
id: gen-direct-address-all-bold
title: 직접 지목 훅 — 공통 · 강조형
title_en: Direct Address — All · Bold
platform: all
category: hook
difficulty: beginner
tags: [hook, direct, bold, all]
author: EZCut
license: Non-commercial evaluation only (see repo LICENSE)
---

# 직접 지목 훅 — 공통 · 강조형

첫 줄에 시청자를 직접 지목하는 자막을 넣어.

## 프롬프트
첫 줄에 시청자를 직접 지목하는 자막을 넣어. 강하고 과감하게 표현해줘.

## 언제 쓰나
설득 콘텐츠에 적합. 조회수의 80%는 첫 3초에 갈린다 — 알고리즘은 초반 swipe-away(이탈률)를 노출 억제 신호로 읽으므로 hook rate를 최우선으로 설계한다.

---

# Direct Address — All · Bold (English)

Open addressing the viewer directly in a caption.

## Prompt
Open addressing the viewer directly in a caption. Express it boldly and aggressively.

## When to use
Great For persuasion. 80% of views are decided in the first 3s — the algorithm reads early swipe-away as a suppression signal, so hook rate is the #1 lever.
